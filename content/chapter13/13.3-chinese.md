...
Haskell是一种纯函数式语言，其属于类[标准ML(SML)](https://en.wikipedia.org/wiki/ML_(programming_language))模型函数语言的一种。与其他已经提过的语言不同，Haskell(或SML)编程是通过函数式进行描述，应用会通过表达式的参数对表达式做出对应的判断。通常，编程的顺序会导致不同的结果。这会使外部声明的值没有进行初始化。这就能看出Haskell类语言的主要优势和主要劣势了。因为Haskell在编程时的劣势很突出，并且其复杂的类型系统，通常会让一些有过C，C++，Java经验的开发者，在第一次使用时，有些难以驾驭的感觉。不过，这些问题会在并行从程序中进行解决，例如我们的这个例子，其表达式计算出来的结果是相互独立的，所以其函数式定义是线程安全的。因此，在Haskell开发者社区中，逐渐涌现出很多有意思的并行程序。对Haskell感兴趣的读者可以读去一下Hutto写的这本使用Haskell编程的书籍[1]，以及Meijer在微软频道9中的相关教学视频[2]。
...
这里我们依旧会使用向量相加作为例子。内核代码不会改变，并且直接嵌入到一个字符串中，而需要改变的则是Haskell主机端代码。
...
很多OpenCL函数需要一个上下文对象，其用来定义OpenCL的执行环境；或是需要一个命令队列对象，提交到队列中的任务将在指定的OpenCL环境中执行。在很多OpenCL代码中，有些参数函数作为“线路噪音”——只是技术上需要，其不会对代码进行很大的修改。为了获得这些信息，HOpenCL提供了两个类类型，Contextual和Queued。这两个类型的实例可以传入相关的OpenCL API中，执行相应的任务。
...
想要向设备提交命令，就需要创建命令队列。queue函数可以通过当前Contextual创建一个命令队列：
...
通过一次调用invoke函数，并不能能够完全将一个内核入队；因此，invoke函数需要和overRange函数一起使用，其会将执行域和结果作为一个是事件进行入队：
